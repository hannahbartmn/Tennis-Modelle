...
# zu vorhersagende Turniere seit 2011 (fuer Vergleichbarkeit mit dem Paper)
all_tourneys <- unique(data[data$tourney_date >= as.Date("2011-01-01") , 1]) #60 Turniere
...
for(j in 1:length(all_tourney_dates)){
  ...
  w_2 <- time_weight(d = temp_subset_4_years$date_difference, halfperiod = 365)
  w_3 <- time_weight(d = temp_subset_4_years$date_difference, halfperiod = 365 * 2)
  w_4 <- time_weight(d = temp_subset_4_years$date_difference, halfperiod = 365 * 3)
  
  # data, pred_subset nicht entfernen!
  rm(na_surface, temp_id_4_years, df_player, df_opponent, all_matches, player_stats,
     X_beta, X_1, X_2, outlier_col, index, outlier, temp_subset_4_years, 
  ...
  w_5 <- time_weight(d = temp_subset_6_years$date_difference, halfperiod = 365)
  w_6 <- time_weight(d = temp_subset_6_years$date_difference, halfperiod = 365 * 3)
  w_7 <- time_weight(d = temp_subset_6_years$date_difference, halfperiod = 365 * 5)
  
  # data, pred_subset nicht entfernen!
  rm(na_surface, temp_id_6_years, df_player, df_opponent, all_matches, player_stats,
     X_beta, X_1, X_2, outlier_col, index, outlier, temp_subset_6_years, 
     X_1_pred, X_2_pred, X_beta_pred, not_listed_opponent, not_listed_player, 
  ...
  save(prediction, file = paste0("GS_O_new_model_prediction_", j, ".RData")) #id hier hinzufuegen
