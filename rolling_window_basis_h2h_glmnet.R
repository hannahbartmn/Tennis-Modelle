...
# for(j in J){
  ...
  rm(na_surface, temp_id_4_years, df_player, df_opponent, all_matches, player_stats,
     X_beta, X_1, X_2, index, outlier, temp_subset_4_years, w_level_4_years, 
  ...
  save(prediction, file = paste0("new_model_prediction_basis_h2h_", j, ".RData")) #id hier hinzufuegen
  ...
  #lambda = 1e-5 
  #penalty.factor fuer die surface x spieler Parameter, nicht fuer die home und h2h variablen!!
  #altes Modell ohne die neuen Variablen auch ueber glmnet laufen lassen 
  #Bookmaker Daten laufen lassen 
