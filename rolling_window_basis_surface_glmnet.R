...
# for(j in J){
  ...
  X_beta <- temp_subset_4_years$home_tourney
  ...
  X_beta_pred <- pred_subset$home_tourney
  ...
  X_beta <- temp_subset_6_years$home_tourney
  X <- cbind(X_beta, X_1-X_2)
  colnames(X)[1] <- "home"
  ...
  save(prediction, file = paste0("new_model_prediction_basis_staerke_nach_untergrund_", j, ".RData")) #id hier hinzufuegen
